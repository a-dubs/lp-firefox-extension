...
function insertInlineCommentElementBelowLine(tr_element, commentElement, line_no) {
    ...
    inlineCommentCard.appendChild(commentElement);
    ...
    inlineCommentContainer.appendChild(inlineCommentCard);
...
function createEditableCommentElement(
    ...
    console.log("initial comment text:", initial_comment_text);
    ...
    saveDraftButton.addEventListener("click", async () => {
        // disable the save draft button and show the custom-spinner
        const btn_text = saveDraftButton.innerText;
        saveDraftButton.innerText = " ";
        saveDraftButton.classList.add("disabled", "custom-spinner");
        ...
        try {
            ...
            // update the draft comment in the DRAFT_INLINE_COMMENTS array and the GROUPED_DRAFT_INLINE_COMMENTS object
            ...
            if (existingDraftComment) {
                ...
                existingDraftComment.commentText = textarea.value;
                DRAFT_INLINE_COMMENTS.push(existingDraftComment);
                GROUPED_DRAFT_INLINE_COMMENTS[preview_diff_id][existingDraftComment.file][existingDraftComment.line_no].commentText = textarea.value;
                ...
                const parsedDraftComment = parseDraftInlineComments(
                    preview_diff_id,
                    [{ line_number_in_original_diff: textarea.value }],
                )[0];
            ...
            // if (editableInlineComment && editableInlineComment.parentElement && editableInlineComment.parentElement.parentElement.classList.contains("d2h-inline-comment-row")) {
            //     editableInlineComment.parentElement.parentElement.remove();
            // }
        } catch (error) {
            console.error("An error occurred:", error);
        }
        finally {
            // always remove the custom-spinner and re-enable the button
            saveDraftButton.classList.remove("disabled", "custom-spinner");
    ...
    postButton.addEventListener("click", async () => {
        // Disable the post button and show the custom-spinner
        const btn_text = postButton.innerText;
        postButton.innerText = " ";
        postButton.classList.add("disabled", "custom-spinner");
        ...
        try {
            ...
            // if editableInlineComment is in a row by itself and not part of a thread of comments, remove the row
            // if editableInlineComment is in a row by itself and not part of a thread of comments, remove the row
            if (editableInlineComment && editableInlineComment.parentElement.parentElement.parentElement.classList?.contains("d2h-inline-comment-row")) {
                editableInlineComment.parentElement.parentElement.remove();
            }
            editableInlineComment.remove();
        } catch (error) {
            ...
            // Always remove the custom-spinner and re-enable the button
            postButton.classList.remove("disabled", "custom-spinner");
    ...
    cancelButton.addEventListener("click", async () => {
        // disable the cancel button and show the custom-spinner
        const btn_text = cancelButton.innerText;
        cancelButton.innerText = " ";
        cancelButton.classList.add("disabled", "custom-spinner");
        try {
            if (cancelButton.classList.contains("delete-draft")) {
            ...
            else if (cancelButton.innerText === "Cancel") {
                ...
                // delete the editableInlineComment element
                editableInlineComment.remove();
                // if editableInlineComment is in a row by itself and not part of a thread of comments, remove the row
                if (editableInlineComment && editableInlineComment.parentElement.parentElement.parentElement.classList?.contains("d2h-inline-comment-row")) {
                    editableInlineComment.parentElement.parentElement.remove();
            ...
            // always remove the custom-spinner and re-enable the button
            cancelButton.classList.remove("disabled", "custom-spinner");
...
function findLineNumberInDiff(fileName, lineNumberInFile, diffTxt) {
    ...
    let addedLines = 0; // Track added lines within a chunk
    let removedLines = 0; // Track removed lines within a chunk

    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];

        // Detect the start of a file path in the diff
        if (line.startsWith("+++ ")) {
            currentFile = line.split(" ")[1].slice(2);
        } else if (line.startsWith("@@")) {
            // Parse chunk header to get starting line numbers for original and new files
            const chunkInfo = line.split("@@")[1].trim().split(" ");
            const newFileInfo = chunkInfo[1].split(",");
            currentLineNumberInFile = parseInt(newFileInfo[0]);

            // Reset added and removed line counters for each new chunk
            addedLines = 0;
            removedLines = 0;

            // Adjust the starting line for new files beginning at 0
            if (currentLineNumberInFile === 0) {
                currentLineNumberInFile = 1;
            }

            currentDiffLine = i + 1; // Track the line number in the diff
            continue;
        }

        // Only process lines within the current file
        if (currentFile === fileName) {
            // If we've reached the target line number in the file
            if (currentLineNumberInFile === lineNumberInFile) {
                return currentDiffLine;
            }

            // Adjust counters based on the type of line in the diff
            if (line.startsWith("+")) {
                addedLines++;
                currentLineNumberInFile++;
            } else if (line.startsWith("-")) {
                ...
                // Unchanged lines increase the current line number in the file
                currentLineNumberInFile++;
            }
        }
    ...
    // If the target line wasn't found in the diff
    return null;
}
...
function parseDraftInlineComments(preview_diff_id, draftComments) {
    ...
    for (const line_number_in_original_diff in draftComments) {
        ...
        parsedDraftComments.push({
            line_number_in_original_diff: line_number_in_original_diff,
            file: file,
            line_no: line_no,
            lineContent: lineContent,
...
function createCommentThreadElement(comments, draftComment) {
    /**
                    ...
                    if (target_gutter) {
                        if (comments.length > 0) {
                            ...
                            insertInlineCommentElementBelowLine(
                                ...
                                createCommentThreadElement(comments, draftComment),
                                parseInt(line_no)
                            );
...
function enable_adding_inline_comments_from_diff() {
    ...
    const gutters = document.querySelector("#custom-diff-area").querySelectorAll(".d2h-code-side-linenumber , .d2h-code-linenumber");
    ...
    for (const preview_diff_id in groupedComments) {
        for (const file in groupedComments[preview_diff_id]) {
            for (const line_no in groupedComments[preview_diff_id][file]) {
                ...
                const commentThreadElement = createCommentThreadElement(comments, draftComment);
...
function createNewEditableInlineComment(tr_element) {
    ...
    const line_number_in_file = parseInt(tr_element.querySelector(".d2h-code-linenumber").innerText);
